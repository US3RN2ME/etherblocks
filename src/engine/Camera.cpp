...
namespace etherblocks::engine {
   ...
   void FpsCamera::move(const glm::vec3& localDirection, float deltaTime) {
      moveAlong(horizontalDirection(right()) * localDirection.x + worldUp() * localDirection.y +
                    horizontalDirection(front()) * localDirection.z,
                deltaTime);
   }
